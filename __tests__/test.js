...
describe("Sports Application", function () {
  ...
  test("Delete a sport ", async () => {
    ...
    console.log(".................", res.body.allSports);
    let csrfToken = extractCsrfToken(res);
    const response = await agent.post("/sports").send({
      title: "Cricket",
      _csrf: csrfToken,
    ...
    const allsportsCount = parsedGroupedResponse.allSports.length;
    const latestSport = parsedGroupedResponse.allSports[allsportsCount - 1];

    res = await agent.get("/sport");
    csrfToken = extractCsrfToken(res);

    const deleteSportResponse = await agent
      .delete(`/sport/${latestSport.id}`)
      .send({
        _csrf: csrfToken,
      });
    const parsedUpdateResponse = JSON.parse(deleteSportResponse.text);
